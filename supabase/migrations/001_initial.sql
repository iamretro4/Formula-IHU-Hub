-- =====================================================
-- Formula IHU Hub - Initial schema (fresh project)
-- Migration: 001_initial.sql
-- =====================================================
-- Single consolidated schema: enums, tables, RLS, triggers, storage, views.
-- Includes user_profiles (with login_approved, university/billing), storage bucket, track_penalties view.
-- =====================================================
...
-- User profiles table (includes university/billing from 015, login_approved from 017)
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    vat_id text,
    login_approved boolean NOT NULL DEFAULT false,
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now()
);
...
-- Function to handle new user registration (login_approved false until admin approves)
...
-- =====================================================
-- STORAGE: team-uploads bucket and RLS (016)
-- Only run if storage schema exists (Supabase Cloud/Local provide it; some SQL runners do not).
-- =====================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'storage' AND table_name = 'objects') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'team_uploads_storage_insert') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'team_uploads_storage_select') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'team_uploads_storage_update') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'team_uploads' AND policyname = 'team_uploads_update') THEN
    CREATE POLICY "team_uploads_update" ON public.team_uploads FOR UPDATE
    USING (team_id = public.user_team() OR public.is_admin())
    WITH CHECK (
      uploaded_by = (SELECT auth.uid()) AND
      (team_id = public.user_team() OR public.is_admin())
    );
...
COMMENT ON VIEW public.track_penalties IS 'Runs that have at least one penalty (cones, off_course, dsq).';
