...
const EVENT_NAME = 'Cost and Manufacturing'
...
export default function CostEventScore({ params, searchParams }: PageProps) {
  ...
  // Cost & manufacturing is now visible to all roles
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        if (active && currentRunId === effectRunIdRef.current) {
          ...
          logger.error('[Cost Manufacturing] Error loading data', err);
        }
      } finally {
        if (active && currentRunId === effectRunIdRef.current) {
          setIsLoading(false);
  ...
  const canScore = role === 'admin' || role === 'cm_judge';
  const canEditAll = canScore; // admin/cm_judge can manage all
  ...
  const onSubmit: SubmitHandler<ScoringFormData> = async (data) => {
    ...
    try {
      ...
      setError(error.message);
      toast.error(error.message);
      logger.error('[Cost Manufacturing] Error saving scores', err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  async function handleApproveAll(bookingId: string) {
    if (!confirm('Approve all scores for this cost & manufacturing slot? This will approve all categories at once.')) {
      return;
    }
    ...
    try {
      ...
      logger.error('[Cost Manufacturing] Error approving all scores', err);
    } finally {
  ...
  async function handleRejectAll(bookingId: string) {
    if (!confirm('Reject all scores for this cost & manufacturing slot? This will reject all categories at once.')) {
      return;
    }
    ...
    try {
      ...
      logger.error('[Cost Manufacturing] Error rejecting all scores', err);
    } finally {
      setApprovingBookingId(null);
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          <Loader2 className="w-8 h-8 animate-spin text-primary mx-auto" />
          <p className="text-gray-600 font-medium">Loading cost & manufacturing event data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6 bg-slate-50/50 min-h-screen animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight leading-none mb-1">
            Cost & <span className="bg-gradient-to-r from-indigo-500 to-cyan-500 bg-clip-text text-transparent">Manufacturing</span>
          </h1>
          <p className="text-gray-400 font-bold uppercase text-[9px] tracking-[0.3em] leading-none">
            Economic Analysis & Production Cost Assessment
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Admin/CM Judge: All Scores with Bulk Approval */}
        {canEditAll && (
          <Card className="lg:col-span-1 shadow-lg border-gray-200">
            <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
              ...
              <CardDescription>
                Approve or reject entire cost & manufacturing slots at once
              </CardDescription>
            </CardHeader>
