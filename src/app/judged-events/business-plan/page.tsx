...
export default function BusinessPlanScore() {
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        if (active && currentRunId === effectRunIdRef.current) {
          ...
          logger.error('[Business Plan] Error loading data', err);
        }
      } finally {
        if (active && currentRunId === effectRunIdRef.current) {
          setIsLoading(false);
  ...
  const onSubmit: SubmitHandler<ScoringFormData> = async (data) => {
    ...
    try {
      ...
      setError(error.message);
      toast.error(error.message);
      logger.error('[Business Plan] Error saving scores', err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  async function handleApproveAll(bookingId: string) {
    if (!confirm('Approve all scores for this business plan slot? This will approve all categories at once.')) {
      return;
    }
    ...
    try {
      ...
      logger.error('[Business Plan] Error approving all scores', err);
    } finally {
  ...
  async function handleRejectAll(bookingId: string) {
    if (!confirm('Reject all scores for this business plan slot? This will reject all categories at once.')) {
      return;
    }
    ...
    try {
      ...
      logger.error('[Business Plan] Error rejecting all scores', err);
    } finally {
      setApprovingBookingId(null);
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          <Loader2 className="w-8 h-8 animate-spin text-primary mx-auto" />
          <p className="text-gray-600 font-medium">Loading business plan event data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6 animate-fade-in min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Admin/BP Judge: All Scores with Bulk Approval */}
        {canEditAll && (
          <Card className="lg:col-span-1 shadow-lg border-gray-200">
            <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
              ...
              <CardDescription>
                Approve or reject entire business plan slots at once
              </CardDescription>
            </CardHeader>
