...
const scoringFields = [
  { id: 'novelty', label: 'Novelty, Mission, Vision', max: 15 },
  { id: 'content', label: 'Content & Concept', max: 20 },
  { id: 'presentation', label: 'Presentation Quality', max: 20 },
  { id: 'financial', label: 'Financial & Sales', max: 20 },
];
...
export default function BusinessPlanPage({ user }) {
  ...
  const fetchData = useCallback(async () => {
    // This is similar to DesignEvent, can be abstracted later
    setLoading(true);
    try {
      const [allTeams, allScores] = await Promise.all([Team.list('name'), BusinessPlanScore.list()]);
      setTeams(allTeams);
      setScores(allScores);
    } catch (error) {
  ...
  const handleSaveScore = async () => {
    ...
    try {
      if (existingScoreId) {
        await BusinessPlanScore.update(existingScoreId, payload);
      } else {
        await BusinessPlanScore.create(payload);
      }
      fetchData();
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Business Plan Presentation Scoring</h1>
          <p className="text-gray-600 mt-1">Submit scores for the BPP event.</p>
        </div>
      </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          This hub is for scoring only. Finals bracket management and presentation timer are not yet implemented.
        </AlertDescription>
      </Alert>

      <Card>
        <CardHeader>
          <CardTitle>Enter BPP Scores</CardTitle>
          ...
          {!selectedTeam ? (
            ...
            <>
              ...
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                ...
                <p className="text-4xl font-bold text-blue-900">{currentScore.total_score} / 75</p>
              </div>
            </>
          )}
        </CardContent>
